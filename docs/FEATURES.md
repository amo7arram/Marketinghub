...
- Validation: a `Published` **SM Content** activity must have a Post Link before it can be saved (link isn't always available for Campaign/Event/Print/Physician Video, so it's only enforced for social media content)
...
### Advisor
- **A personal briefing over your own data**, deliberately calmer in layout than the operational tables. Available to all admins. Replaces AI Reports in the nav (AI Reports is hidden, code kept — see below).
- **At a glance (the "pulse")** — computed entirely client-side from data already loaded, no AI, instant: busiest team member this month, leads this month vs. last (contacted %, booked %, conversion delta, top closure reason, slowest-contacted source), spend month-to-date vs. budget pace + biggest line item, biggest month-over-month social mover/decliner, comment sentiment (from the Dashboard Metrics tab's Comment Sentiment section, see below), hygiene (overdue / stuck-in-production / unassigned), the next health day on the calendar, and the most under-served department (high lead demand, no campaign this quarter).
- **The briefing** — one Claude call (`claude-opus-5`) turns that pulse plus a sample of **redacted** call-centre notes into: a headline, "what stands out," what the phones are hearing, 2–3 ranked recommendations (why / do / priority), a watch-list, campaign ideas tied to seasons/health days, and one contrarian provocation. Cached in `config/advisor_brief` with a "generated by X · 3h ago" stamp; any admin can regenerate.
- **Call-centre notes are PII-redacted before the call** — each lead's own name/phone/email is stripped from its note, plus generic email/number patterns; the briefing reports coverage ("34 of 61 recent leads carried a note or closure reason").
- **Ask your advisor** — an in-memory chat box (not persisted) that answers questions against the same pulse snapshot.
- Same "numbers computed in JS, model supplies judgment only" discipline as AI Reports. Every AI call is a direct browser call to the Anthropic API (existing key in `config/ai_settings`).
- **Phase 2, not built yet:** a scheduled GitHub Action to crawl the imc.med.sa doctor roster (→ doctor-level campaign recommendations) and run a competitor-news scan. This tab currently gives department-level campaign gaps only.

### AI Reports *(hidden from nav, code intact — revisit later)*
...
- **Campaign Performance panel** — collapsible breakdown, one row per campaign with linked leads: Leads, Cost, CPL, Booked, CPA, ROI. Computed by the same `computeCampaignStats()` used to publish the public Leads & Attribution dashboard's Campaign Performance card, so the two always agree. Click a row to jump to that campaign's filtered leads table.
- **Bulk Edit** — Department/Entity/Source/Contact Status/Outcome/Date, plus Campaign (bulk-attach leads to a campaign) and Assign to Agent, applied only to fields explicitly set
...
- **Email** — optional field, captured via the manual Add/Edit modal; every landing-page-captured lead has one, most others don't (not yet part of Excel/Sheet import column-detection). Hidden from the table display for now (still stored and editable via the lead modal)
...
- Excel upload with automatic column detection (name/phone/department/entity/source/campaign/date/status/notes), department fuzzy-matching, a campaign-attribution picker, and an optional "Assign to Agent" picker (applies to every imported row). Deduplicates by phone+campaign combination (not phone alone) against existing leads — recomputed live as the campaign dropdown changes, since that's the only input the dedupe key depends on that isn't fixed at parse time; duplicate rows are shown dimmed in the preview and skipped on import, with the skip count called out. Rows with no date column detected in the sheet default to today's date rather than being left blank; a row with a date column present but an unparseable value is still flagged "not parsed" in the preview, since that's a data-quality signal worth surfacing rather than silently masking
...
- Same "🔄 Sync Now" also matches every `Published` initiative's `postLink` against Metricool's own scheduled posts and writes a real matched media URL to `initiatives.socialImageUrl` where found (see `docs/DATA_MODEL.md`) — powers the public Dashboard's "this month" campaign thumbnails and the Recent Posts gallery below. Prefers a still image but falls back to the raw `.mp4` for reels/video posts (the public dashboard renders those as a muted, seeked video frame instead of an `<img>`, so reels aren't excluded). Partial coverage by nature (only Metricool-scheduled content can match), never surfaced as an error.
- **Recent Posts** (public Dashboard) — an image gallery pulling the same `socialImageUrl`/`postLink` pairs, scoped to the dashboard's selected month (same period nav as the KPI/Marketing Output/Leads sections above it)

### Comment Sentiment *(part of Dashboard Metrics, admin-only)*
- **"🔄 Sync Comments & Reviews"** pulls real comments/reviews via Metricool's own Inbox API — Instagram, Facebook, and TikTok comments (`GET /v2/inbox/post-comments`), plus Google Business reviews with their star rating (`GET /v2/inbox/reviews?provider=GMB`). Confirmed against Metricool's own `swagger.json`, not scraped and not a third-party tool — the same official API this app already uses for everything else Metricool.
- Each new item is classified by Claude (`claude-sonnet-4-6`) into positive/neutral/negative plus up to 2 short theme tags (e.g. "wait times", "pricing"). Already-classified items are never re-classified on a later sync, so re-syncing doesn't re-spend AI tokens or double-count.
- **Built for a monthly cadence**: the summary shows the current calendar month's % positive/neutral/negative breakdown with the point-change vs. last month, common themes for the month, and a scrollable list of that month's real comments/reviews (negative ones surfaced first) — each showing the platform (Instagram/Facebook/TikTok/Google Business, icon + name), the date, author name, star rating (Google Business), and sentiment label. Filter chips above the list narrow it to one platform at a time (client-side, no extra load).
- **Coverage caveat**: Metricool's Inbox API doesn't support requesting a specific date range, so one sync isn't guaranteed to capture every comment for the month — sync a few times through the month rather than relying on a single sync at month-end. Numbers still accumulate correctly regardless, since every comment is bucketed by its own real date whenever it's fetched.
- **Admin-only, deliberately** — this section is never shown on the public dashboard. The comments were posted publicly, but some are patient-adjacent complaints; re-publishing those on IMC's own site would be amplifying someone's public complaint in a context they didn't choose.
- Also feeds the Advisor tab's pulse (see above) as a "Comment sentiment" card (with the same month-over-month point-change) and briefing input.
- Storage cost is a non-issue at any realistic comment volume — see `docs/DATA_MODEL.md`'s `social_comments` section for the numbers.
...
- **Fast-loading by design**: unlike every other page in this app, `landing.html` never loads the Firebase SDK (Auth included) — it talks to Firestore's REST API directly, since a slow-loading page on a cold ad click directly costs conversions. A shimmer skeleton renders instantly (before any script runs) while content loads. See `docs/ARCHITECTURE.md` §4.6.
