...
- Renders hero image (if set) / headline / subheadline / body content / CTA button text from the fetched page, then a Name / Email / Phone / optional Comment form (the comment lands in `leads.notes`). Body content is sanitized (DOMPurify) before rendering — see the Landing Pages admin section above
- **Bilingual form prompt** — the form title ("Fill the form to receive a call") and the scroll nudges below are shown in English and Arabic together (IBM Plex Sans Arabic, RTL)
