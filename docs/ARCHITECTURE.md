...
| Database | Firebase Firestore (NoSQL document store) — accessed via the JS SDK everywhere except `landing.html`, which talks to Firestore's REST API directly for load-time reasons, see §4.6 |
...
| Analytics | Google Analytics (GA4), optional, consent-gated — `landing.html` only, see §4.6 |
...
├── offers.html            Public, no login, the Excel-driven Offers Catalog that replaced the
│                          retired Promotions feature — same REST-only fast-load approach as
│                          landing.html, but a fuller branded page shell (nav bar, hero, footer),
│                          since this one is meant to feel like a standalone public destination
...
**Super Admin is deliberately *not* a role.** The "reassign any activity regardless of creator" capability is a plain `superAdmin: true` boolean on the person's `team_members` row, checked client-side in `admin.html` (`iAmSuperAdmin()` / `canReassign()`). It was kept out of the `roles` system on purpose: adding a fourth role would mean widening the `role === 'admin'` login gates in `admin.html`, `actions.html`, and `call-center.html` (and the `roles`-write rule) or else locking the holder out, for a feature that only needs a soft UI check. The flag is soft-limited to one holder (a second one is blocked in the Team Members editor, not by any rule). If the team ever moves fully off the Magic Word shared login onto individual accounts, promoting this to a real enforced role + Firestore rule is a clean follow-up.
...
### 4.4 The "Magic Word" Passcode — removed

**Removed once every admin had an individual email/password account.** This was a parallel login path on `admin.html` (and, reusing the same code, `actions.html`): entering a shared passcode triggered a silent Firebase Auth login as one hardcoded designated account. It never weakened Firestore security (a real auth session was established either way), but it meant everyone who used it shared one identity — incompatible with the per-user attribution the assignment/collaboration features need, and it kept a hardcoded credential in the committed source of two files.

What went: the login UI on both pages, `switchLoginMode`/`doMagicWordLogin`/`saveMagicWordPasscode`, the `MAGIC_WORD_ACCOUNT_*` constants, `getAdminPasscode`/`setAdminPasscode` in `firebase-data.js`, the Settings → "Admin Login Passcode" card, and the `config/admin_passcode` rule. Both pages now use plain email/password only. The `config/admin_passcode` **document** may still sit in Firestore, unread and (with its rule gone) default-deny — harmless to leave, safe to delete in the Console.

Unrelated and untouched: the **Public Access Gate** (§4.5, `config/access_gate`) — a different mechanism, different document, still in use on the public portal. Note the hardcoded passcode account's password (`anaggar@imc.med.sa`) remains in git history; removing the code does not retire that credential, so it should be rotated in Firebase Auth.
...
**Google Analytics, consent-gated.** `landing.html` can optionally load GA4 (Measurement ID set once in `admin.html` Settings, `config/analytics_settings` — see `docs/DATA_MODEL.md`), off by default until an admin configures it. Even once configured, the GA script is never loaded on page load — a cookie-consent banner appears first (linking to IMC's real Privacy Policy & Cookie Notice at `imc.med.sa`), and GA only initializes if the visitor clicks Accept. The choice is remembered in that browser's `localStorage` (never transmitted anywhere) so the banner doesn't reappear on a later visit. Only a `generate_lead` conversion event (landing page slug, campaign, department — no name/email/phone) and standard GA page-view data are ever sent; it fires only on a genuine successful submission, never on the honeypot/timing spam-fake-success path, so bot traffic doesn't pollute conversion numbers. The Terms & Conditions and Privacy Policy & Cookie Notice links in the page footer point to IMC's official website rather than a copy hosted here, deliberately — a duplicated policy that drifts out of sync with the real one is worse than no duplicate. **None of this constitutes legal sign-off**: whether this flow satisfies Saudi PDPL or any other applicable regulation is a question for IMC's compliance function, not something this document or its author can certify.

**`landing.html` bypasses the Firebase SDK entirely — a deliberate exception to "every file imports from `firebase-data.js`."** Real incident: a visitor reported ~25 seconds of blank loading before content appeared on a Meta ad click (likely Instagram/Facebook's in-app browser over cellular). Root cause: `firebase-data.js` unconditionally initializes Firebase Auth alongside Firestore (`export const auth = getAuth(app)`), since every other page in the app needs it for login — but a landing-page visitor is never authenticated, so that page was paying to download, parse, and initialize an entire Auth SDK it could never use. The fix: `landing.html` talks to Firestore directly over its plain REST API (`https://firestore.googleapis.com/v1/projects/{id}/databases/(default)/documents/...`) instead of importing the SDK at all. **Security is unaffected** — Firestore Security Rules are enforced server-side identically regardless of transport (SDK, REST, or gRPC), so the exact same `landing_pages` get-rule and `leads` anonymous-create whitelist (§ above) apply unchanged. The tradeoff, accepted deliberately: `normalizePhone()` and the Firebase project config are now duplicated in `landing.html` (marked with a cross-reference comment in both files) rather than shared, and a hand-rolled `fsUnwrap`/`fsString` pair replaces the SDK's automatic type marshaling for Firestore's typed-field wire format — a small, bounded amount of extra code to maintain in exchange for not shipping an unused database client library on the one page where load time directly costs ad conversions. DOMPurify (`defer`red) and a shimmer skeleton screen (visible before any script even runs) round out the fix — neither reduces actual latency, but a skeleton with visible motion reduces the "is this broken?" abandonment reflex during any residual wait.

### 4.7 Offers Catalog — retiring Promotions in favor of an Excel-driven public catalog

Finance built a standalone site (`imcoffers.com`) from their own master Excel price/discount sheet, separate from this app's manually-entered Promotions feature. Rather than maintain two parallel, hand-updated offers destinations, the Promotions feature (admin.html's tab, index.html's Promotions Calendar page, and its `isWellspan` integration on the Well-span page) was retired entirely and replaced with `offers.html` — a single canonical, Excel-driven catalog intended to eventually be what `imcoffers.com` redirects to.

**Data model**: `offers_catalog` is sharded one Firestore document per branch (doc ID = a slugified branch key), not one collection-wide document — a deliberate scaling precaution, since Firestore documents cap at 1 MiB and the real ongoing catalog is very likely larger than the 287-row National Day campaign subset (`national-day-offers.html`) this was modeled on. A small `config/offers_catalog_meta` document lists which branches exist (with labels and counts) so `offers.html`/`call-center.html` can render tabs immediately from one read, before fetching any branch's full offer list.

**Rules are simpler here than `landing.html`'s** — only an authenticated admin ever writes (via the Excel import), so this needs none of the anonymous-create complexity §4.6 describes: plain `allow read: if true; allow write: if request.auth != null;` on both `offers_catalog/{branchId}` and `config/offers_catalog_meta`, the same pattern already used for `initiatives`/the retired `promotions`.

**The Excel column-detection was originally designed without access to Finance's actual live sheet** (`imcoffers.com` was blocked by this network's FortiGuard filter, and no sample file was available at build time), leaning on `national-day-offers.html`'s schema as a best guess. **It has since been verified against Finance's real "Promotional Campaigns" sheet** and rebuilt around what that sheet actually contains — see `DATA_MODEL.md`'s `offers_catalog` section for the specifics (the `Status` filter, the `Provider`-code branch mapping and its comma-separated multi-branch cells, the Gross-Price back-calculation fallback, `Campaign Title` as category, and AI-drafted Arabic). The mandatory preview step (detected columns, status-excluded counts, per-branch/category counts, price-skip examples, unrecognized-branch warnings, Arabic sample) remains the safety net before anything publishes — the column mapping is now real, but the preview still catches anything Finance's sheet does differently next time (a renamed column, a new branch code, a new status value).

### 4.8 Comment Sentiment — Metricool's real Inbox API, not scraping

The request was to add sentiment analysis on social comments. Metricool itself has no sentiment feature (confirmed via their own help docs before building this), and the naive approach — scraping Instagram/Facebook/TikTok directly — would risk the real business accounts' standing and wasn't necessary anyway.

**Metricool's Inbox (their own comment/review/DM management feature, visible in their web app) is backed by a real, documented API**, verified directly against their live `swagger.json` rather than assumed: `GET /v2/inbox/post-comments?provider={INSTAGRAM|FACEBOOK|TIKTOKBUSINESS}` for comments, `GET /v2/inbox/reviews?provider=GMB` for Google Business reviews (which come with a `stars` rating attached, no AI needed to know a 1-star review is bad — though it's still classified for theme extraction). Both go through the existing Metricool CORS proxy Worker described above with zero Worker changes needed, since it's a generic path-forwarding passthrough. This is the same authenticated API surface this app already uses for analytics/scheduling — no new Meta Developer App, no App Review process, no scraping. (Metricool's schema also allows `INSTAGRAMBUSINESS` for the Instagram row; confirmed live that this account's connection needs plain `INSTAGRAM` instead.)

**Sentiment itself is Claude, not a pretrained model.** Comment text here is realistically mixed English/Arabic — a generic off-the-shelf sentiment classifier (the kind of thing a GitHub project would offer) handles that far worse than an LLM does. Same direct-browser-call pattern as every other AI feature in this app (`claude-sonnet-4-6`, chosen over `claude-opus-5` since this is high-volume simple classification, not the judgment-heavy work opus is reserved for elsewhere).

**Admin-only, deliberately — a product decision, not a technical PII rule.** The comments were posted publicly, so there's no new privacy exposure in an admin seeing them (same trust level as leads/agent notes already shown there). But some are patient-adjacent complaints, and republishing those onto IMC's own public dashboard would be amplifying someone's public complaint in a context they didn't choose to be featured in — reputational, not legal. `social_comments` and `config/sentiment_stats` are both authenticated-read/write, and neither is read by any public page.
...
- **Client-side AI API key exposure.** The Anthropic API key is stored in Firestore and used directly from the browser. Anyone with admin login and developer tools open can see it in network requests. Acceptable for a small trusted team; not acceptable at scale. The Advisor tab (`admin.html`) leans further on this pattern — its briefing and chat are direct browser calls, and its briefing sends redacted call-centre notes to the API. Both the key exposure and the AI-proxying belong in the first Cloud Function whenever the backend tier lands.
- **Advisor "background" work isn't actually background.** The Advisor tab computes its pulse and generates its briefing on demand in the browser. The parts that genuinely need a scheduler — crawling the imc.med.sa doctor roster, a competitor-news scan — are deferred to a planned GitHub Actions workflow (the repo's first CI), which would write to Firestore docs the tab already reads.
