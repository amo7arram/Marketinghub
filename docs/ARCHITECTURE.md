...
**The Excel column-detection was designed without access to Finance's actual live sheet** — `imcoffers.com` is blocked by this network's FortiGuard filter (an "Unrated domain" block, confirmed live, unrelated to the site itself), and no sample file was available at build time. The import leans on `national-day-offers.html`'s exact schema (`code`/`description`/`billgroup`/`price`/`discount_pct`/`discounted_price`/`arabic`/branch, very likely the same Finance data pipeline) as its best guess, with a mandatory preview step (detected columns, per-branch/category counts, skipped-row count) as the safety net before anything publishes. **This should be verified against Finance's real current sheet before being treated as fully trustworthy.**
