...
**`lastSyncReport`** is the per-platform breakdown from the most recent sync (item count, or the failure reason) — shown persistently under the sync button, not just as a transient toast. Added after a real incident where every platform except Google Business silently returned nothing, with no visible indication of *why* (a per-provider fetch failure was only ever `console.error`'d, invisible to the admin). Currently tries **both** `INSTAGRAM` and `INSTAGRAMBUSINESS` for the Instagram row — a real sync confirmed `INSTAGRAM` is the one that returns data for this account (`INSTAGRAMBUSINESS` returned 0); `INSTAGRAMBUSINESS` stays in `commentChannels` (`fetchInboxItems()` in `admin.html`) for now only as a harmless always-empty check, worth dropping in a later cleanup pass.

**A second, more serious bug found via the same real sync**: `/v2/inbox/post-comments` returns one `PostCommentsThread` per top-level comment, whose `root` field (type `RootComment`) **is that comment itself** (`root.text`/`root.owner`/`root.creationDate`) — `root.comments[]` is only *replies* to it, which is empty for the large majority of real comments (most comments never get a reply). The original code only ever read `root.comments[]` and never `root` itself, so it silently discarded almost every real comment — 140 fetched Instagram threads produced close to zero usable items. Fixed to extract `root` as a comment in its own right, in addition to any genuine replies in `root.comments[]`. A temporary "🔍 Raw sample per platform" debug view (collapsed by default, under the recent-comments list) now shows one unmodified raw thread/review per channel after every sync, specifically so a mismatch like this can be confirmed against real data next time instead of guessed at blind.
