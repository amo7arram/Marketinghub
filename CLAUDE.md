...
- **Four separate HTML entry points, each its own login gate:** `admin.html` (role: admin), `call-center.html` (role: agent or admin), `request.html` (role: coordinator or admin), `index.html` (public, no login by default). This separation is deliberate — don't merge them. **`actions.html` is a fifth, documented exception**: a read-only Marketing Actions view for executive leadership that deliberately reuses `admin.html`'s exact role="admin" session rather than getting its own role — see `docs/ARCHITECTURE.md` §4.4 for the access-model tradeoff this accepts.
